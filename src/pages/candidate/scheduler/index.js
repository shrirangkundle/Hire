...
import candidateApis from "../candidateApis";
...
function Candidate(params) {
    ...
    const [jobId, setJobId] = useState(staticData)
    ...
    useEffect(() => {
        ...
        let jdId = new URLSearchParams(search).get('jdId')
        let recruiterId = new URLSearchParams(search).get('recruiterId') 
        formatData(activeRange, jdId)
    ...
    const formatData = (range, jdId) => {
        const start =new Date();
        const end = new Date(moment(start).add(range-1, 'day'));
        let loop = new Date(start);
        const getDate = async() => {
            let api = candidateApis["GET_CANDIDATE_SLOTS"]
            let userD = {
                candidateId: userDetail.userIdentifier,
                jdId: jdId
    ...
    const addMeetings = (params) => {
        let modData = [...data]
        let api = candidateApis["BOOK_CANDIDATE_SLOTS"]
        let postData = {
            panelistId:params.extraInfor.panelistId,
            slotId: params.extraInfor.slotId,
            candidateId:userDetail.userIdentifier,
            jdId: jobId,
            recruiterId: recruiterId || params.extraInfor.recruiterId || ""
        }
        if(params.operation == "candidate-cancel") {
            api = candidateApis["CANCEL_PANELIST_SLOT"]
            postData = {
                slotId: params.extraInfor.slotId,
